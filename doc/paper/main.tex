...
%    and average in last. Residual measured as $\sqrt{MSE}$ with noisy signal,
%    Error measured as $\sqrt{MSE}$ with true (noiseless) signal.}
%\label{tab:LowNoiseResults}
%\end{table}

The minimum and maximum error particles had MSE of $ $ and $ $,
respectively, while the mean of the particles gave MSE of $ $. 
%TODO fill in errors
The correlation matrix for the final parameter estimates is shown in 
\autoref{tab:long_corr}.
The significant correlation of parameters indicate that no one estimate of 
particles dominate the solution. Additionally, given the very low error in 
the BOLD,
the energy graph is full of global minima. 

The final histogram for a single active region, 
...
%parameters:
%&1.151&  0.353& 0.380& 0.026& 1.98&  2.333& 1.645 \\
...
{\includegraphics[width=2.5in]{images/simregions.png}\label{fig:simslice_mask}}
\hfil
\subfloat[Mutual Information Map. Scale is bits. Higher (yellow) is better.]
{\label{fig:sim_hm_mi} \includegraphics[width=2.5in]{images/sim_hm_mi}}
