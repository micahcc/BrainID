...
expected state progression. This voxel is not being driven directly by the input, although it
...
The fourth voxel (\autoref{fig:comp4}) selected for analysis had a relatively high residual, and was not picked
...
being driven by the input. It is likely that the cause of the activation in this reason is
not true activation, but received from surrounding active regions through spatial smoothing. 
...
more by the pre-processing methods than the regression method. The reason for 
SPM applying such extensive smoothing is to combat false positives. While this is
...
fit the measurements; and at the very least this method is a workable, albeit
