...
that distribution. Because the distribution is continuous, particle impoverishment
cannot occur. The easiest way to sample from the continous distribution is to break the 
re-sampling down into two steps. After calculating an estimate of the scale of the original
distribution, \autoref{alg:Resampling} is performed. Next, a distribution is generated
based on the original distributions, although it should have a somewhat smaller variance.
Finally, for each particle in the resampled distribution, a sample is drawn from the smoothing 
distribution and added to the particle.  The process of the selection will be defined as:
...
also espouses the usefullness of the Gaussian Kernel, due to the ease
...
\STATE Resample $\hat{x}$ using \autoref{alg:Resampling}
\FOR{$0 < i < N_p$}
...
Thus, with regularized resampling, it is possible to prevent both
...
random steps get smaller and smaller. On the down side,
if for some reason the real solution was not covered by the new support 
generated by the resampling algorithm the algorithm could 
