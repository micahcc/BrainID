...
These same parameters were used for all the tests in this chapter. The noise realizations were
...
The preprocessing again led the estimate
to somewhat higher activation levels, and it would appear that the subtleties of
different time constants are being lost due to the noise. 
...
undershoot better. Its worth noting that the post-stimulus undershoots
here are much shorter than the observed prolonged post-stimulus undershoot
...
being smaller than the measurement noise ($0.005$ vs. $0.01$). The error
certainly suffers due to this effect.

%NO SIGNAL, LOW NOISE
\subsection{Pure-Noise, low magnitude}
\label{sec:PureNoiseLowMag}
The next two single-voxel tests force the particle filter to attempt to learn a noise-only
time series. In this first test the noise used is the same as that from the \autoref{sec:SimHighNoise},
$\sigma_x = 0.01, \sigma_y = 0.005$. The parameters will be set the same as well, but the
