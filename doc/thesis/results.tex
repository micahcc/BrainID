...
\hline 
1.0324 & 0.33211 & 0.34058 & 0.03012 & 1.40665 & 2.52079 & 0.5311 & todo \\
...
\caption{Mutual Information and the normalized $\sqrt{MSE}$, 
    for signal/noise configurations. TODO FIX MIN/MAX/MEAN}
...
\begin{equation}
\sum_{x,y} p(x,y) \log_2\left(\frac{p(x,y)}{p(x)p(y)}\left)
\end{equation}
...
distinguishing between these cases with normalized residual alone is difficult.
On the other hand the minimum mutual information in the high noise case was $.09077$,
and the maximum mutual information in the no signal, high noise case is $.03163$. This
is a decent break, and should provide a means to  detect what would otherwise be a 
false positive by the normalized residual. That said, mutual information is in some 
ways more permissive than residual based metrics. For instance, the addition of an arbitrary
constant to a random variable does not effect mutual information. For this reason, 
throughout the rest of the results, both values are reports. 
...
of the particle filter at identifying regions of activation; which usually
...
research. Tighter thresholds would certainly remove the false positives present
in the images. Interestingly the false positives present in the Mutual Information
map are different from those in the residual map. For this reason, it is 
possible that the best solution would make use of both metrics. Although
at first glance it would appear that there are false negatives; this is
