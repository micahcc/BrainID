...
be found in \autoref{tab:flashes}, but because of the availability of ground
...
To test how well the particle filter would do plenty of data, and 
determine the inherent variance in the model parameters, a very long simulation
with randomly generated impulse stimulus was created. The preprocessed timeseries
...
This is in spite of the noisy input (green line in \autoref{fig:long_converge}).
...
The following tests all use less data, on par with the average length of an FMRI
session.  For this low noise case, the eleven realizations are shown in 
...
effective at removing trends. The spline did struggle to match the signal 
...
A similar circumstance occurs at 100 seconds in. A combination of 
...
Clearly the additional noise resulted in much more sporadic results. This is often the 
...
portions of the brain, this section repeats the previous with much higher
noise peaks. To simulate this case another
pure noise signal was generated using $\sigma_x$ of $.1$ and $\sigma_y$ of $0.05$.
...
occur if the prior happens to miss the solution. 
...
signal (\autoref{fig:justbignoise_fit_0}). The preprocessing also causes the particle filter
to converge to a non-zero response in spite of the fact that the input does not correlate
with the stimuli in any way (\autoref{fits_noiseonly_high}). 
