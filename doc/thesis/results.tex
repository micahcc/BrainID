...
point. The patch for POSSUM will be made available. As side effect of 
not using POSSUM's original activation scale, the BOLD \% difference levels 
were in the range of 50 - 100\% from the base, about 5 times as large as they 
should have been. I therefore manually added 750 to the total level of
simulated POSSUM images, putting results into the 1 - 5\% range. This 
had no effect on the quality of the simulation and should not effect the 
underlying parameters (other than $V_0$). 
Also worth noting is that the noise level was set to an SNR of 20, due to changes 
in the program the true signal-to-noise ratio was much lower, as seen in the 
fit in \autoref{fig:simslicefit1}.

%todo where is this from?
\begin{figure}
\centering
\includegraphics[clip=true,trim=5cm 2cm 4.5cm 2cm,width=15cm]{images/simslice_noise}
\caption{A single time-series fit in the simulated slice. Preprocessed BOLD is
the noisy response generated by POSSUM.}
...
%todo quantify the number of false positives, add extra Heatmap removing false positives
