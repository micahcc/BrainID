...
efficacy, $\epsilon$ is a sutiable variable for estimating voxel 
...
Particle filters, also known as Sequential Monte-Carlo methods (SMC),
are essentially an on-line version of the Markov-Chain Monte Carlo (MCMC)
algorithm. Similar to MCMC, Particle Filters assume the Markov Assumption
holds, in essence that 
\begin{equation}
P(X_t | X_{t-1}, X_{t-2}, ... X_{0}) = P(X_t | X_{t-1})$
\end{equation}
for a time evolving parameter, $X_t$. This is actually a very good
assumption in most biological systems, since chemicals driving physiological
parameters rarely have memory. Unlike MCMC algorithms, particle filters
only calculate the posterior distribution:
\begin{equation}
P(X_t | y_t, y_{t-1}, ... y_0)
\end{equation}
for $t$ measurements, $y_t, ... y_0$. This frees the algorith from having
to calculate, or even keep track of the values of $X$ at times other than
$t$, and significantly reducing computation time. This also easily lends
the particle filter to solving stochastic differential equations (SDE's). 
