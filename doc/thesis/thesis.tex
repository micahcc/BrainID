...
The unscented kalman filter is a powerful bayes-gaussian filter that attempts
...
In order to estimate the posterior at $t$, a deterministic set of points (often 2 
per dimension, plus 1 at the mean)
...
The difficulty with the Kalman filter is that it is not well suited
to parameter estimation. If we wanted to find the mean of a static
parameters, such as $V_0$, it would be necessary to guess a mean 
and variance for all time. This is because the Kalman filter is primarily
concenrned with estimating the output, and is not really concerned
with estimating the underlying state, $X(t)$. As such estimated 
output, $\hat{Y}(t)$, is simply a weighted sum of the measurement,
$Y(t)$, and the expected value of the output based on the propagated
state variables, $\hat{X}(t)$. As such, when the  the filter will attenuate measurements
the vastly disagree with $E[h(\hat{X}(t))]$, but eventually 
...
than the full Particle Filter we discuss in the next section. 
...
So a simple example may be used to demonstrate the proposed use of a particle filter.
...
Thus when $u(t)G$ is less than $v_t$, the circuit will discharge the capacitor,
but when $u(t)G$ is greater than $v_t$, the capacitor will charge up. Unlike
the normal rectifier circuit, we'll deal with a generic input
Let $V_C$ be the 
