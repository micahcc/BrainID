...
\begin{equation}
\label{neff}
N_{eff} \approx \frac{N_p}{\sum_{i=1}^{N_p} w_i^2}
\end{equation}
...
a support more suited to the posterior. Thus, if weights are all set to $1/N_p$, and 
$N_p$ points are drawn from the posterior,
\begin{equation}
\hat{x}_j \sim \left(\sum_{i=1}^{N_p} w_i(t)\delta(x - x_i(t))\right), j \in \{1, ..., N_p\}
\end{equation}
then $\hat{x} \sim x$. Unfortunately this isn't necessarily the truth: since the support is
still limited to the original particles, the number of unique particles may only go down.
This effect, often dubbed "particle impoverishment" can result in excessive quantization
errors in the final distribution. However, by instead sampling from a smoothed version of
the discrete distribution, sample impoverishment is prevented. The question then is how to 
decide on the smoothing kernel. 
...
have the same weight, as is the case after basic re-sampling, the kernel that minimizes the 
...
may still be a good choice. Its worth noting that as $n_x$, the number of dimensions in
$x$ go to infinity, the standard deviation based approximation becomes inefective according
 to (cite a Tutorial on Particle Filters for on-line non-linear non-gaussian bayesian
 tracking, sanjeev arulampalam, simon maskell, neil gordon...). Because our regularized 
 resampling method is based on particle variance, resampling very often will slow down 
 convergeance, because the resampling itself can lead to increased variance. Thus, for
 the most part the resulting algorith is the basic particle filter, except for when the
 $N_{eff}$ drops very low (say less than 50), at which point regularized resampling is 
 performed. The resulting algorthm is as follows:
...
\FOR{$t$ in $T$}
    ...
    \IF{$N_{eff} < N_R$ (recommend $N_R = min(50, .1N_p)$ )}
        ...
        \STATE Redraw particles using basic resampling
        \FOR{$i$ in $N_p$}
...
or a genetic algorithm. "Fit" versions of $x$ spawn more children nearby which allow
...
\subsubsection{Choosing $\hat{Pr}(y(T) | x(T))$}
Analysis of Gaussian versus exponential

Discussion of optimality (or lack thereof)

Discussion of using a gaussian mixture pdf to describe a non-gaussian distribution
