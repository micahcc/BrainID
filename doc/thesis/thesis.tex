...
an initial mixture PDF obviously introduces bias; however, every gradient
descent must choose starting points and it is often quite
...
in $y[0:T]$. For some tests I will use de-trending methods to reduce the effects of 
$\nu_x$, the remainder of which gets re-factored into $\nu_y$. Both $\nu_x$ and $\nu_y$
...
The goal of the particle filter to evolve a probably distribution $\hat{x}(T)$,
that asymptotically approaches the probability distribution of $x(t)$,
given known inputs, $u[0:T]$ and noisy readings $y[0:T]$.
To begin with, the particle filter starts with a prior distribution,
