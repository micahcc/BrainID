...
It is will known that Deoxyhemoglobin acts as a contrast agent in EPI imaging
\cite{Buxton1999}, \cite{WEISSKOFF1994}, \cite{Ogawa}, so then what causes
changes Deoxyhemoglobin in a voxel? Obviously increased metabolism will increase
Deoxyhemoglobin, but changes in blood volume in a voxel will also alter 
the MR signal depending on the ratio of Deoxyhemoglobin to hemoglobin. 
This was the impetus
for the ground breaking research \cite{Buxton1999} and \cite{Mandeville1999}.
...
to show that a simple, well chosen blood flow waveform, along with a balloon 
model, could fully account for the BOLD signal. Although complex
changes in metabolism exist (\cite{Zheng2002}), most recent studies have
...
and later \cite{Riera2004} showed that the primary signal may be accurately
be estimated by a simple flow-locked expression of the CMRO2. Comparison
between the more complicated models including a separate expression for CMRO2
and the simplified Buxton expression show little actual difference \cite{Deneux2006}.
In \cite{Buxton1999}, it was shown that small changes in the flow expression could
result in vastly different recovery times of the baseline BOLD signal, and
in \cite{Mandeville1999} a cause for delayed CBV recovery was proposed, based
on differences in compliance of capillaries. \cite{Behzadi2005} provided a simple,
complete model and explanation for delayed compliance of neural capillaries,
and those simple changes can result in a significant improvement in the bias error,
as shown in \cite{Deneux2006}. In the end, it may be impossible to refute the
