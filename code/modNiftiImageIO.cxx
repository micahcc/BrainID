...
modNiftiImageIO::modNiftiImageIO():
  m_NiftiImage(0),
  m_RescaleSlope(1.0),
...
void modNiftiImageIO::Read(void* buffer)
{
  void *data = 0;
  ...
  if(numComponents == 1 || 
    ...
    if(this->GetPixelType() == ImageIOBase::DIFFUSIONTENSOR3D ||
      ...
      {
//      vecOrder = _LowerToUpperOrder(_SymMatDim(numComponents));
      vecOrder = _UpperToLowerOrder(_SymMatDim(numComponents));
      }
    else
      {
...
void _readkey(MetaDataDictionary& dict, std::string key,
...
::ReadImageInformation()
{
  ...
  for(int ii=0 ; ii<this->m_NiftiImage->num_ext ; ii++) {
    if(extent->ecode == INVALID_ECODE) {
      std::string description(extent->edata);
      //two null bytes, and esize and ecode make 10 extra bytes
      _readkey(this->GetMetaDataDictionary(), description, 
            (unsigned char*)extent->edata+description.length()+1);
    }
    //cast as a char star to move forward bytes, rather than ints
    extent++;
  }
...
{
  if(bufflen < 18) {
    throw(std::string("Error buffer not long enough in modNiftiImage::writekey"));
  }
  struct {
    char label;
    unsigned char length;
  ...
  std::string tmps;
  if(dict[key]->GetMetaDataObjectTypeInfo() == typeid(tmps)) {
    itk::ExposeMetaData<std::string>(dict, key, tmps);
    head.label = 'C';
    head.length = tmps.length()+2 > bufflen ? bufflen-2: tmps.length();
    memcpy(buffer, &head, 2);
    memcpy(buffer+2, tmps.c_str(), (size_t)head.length);
  } else {
    if(dict[key]->GetMetaDataObjectTypeInfo() == typeid(data.tmpd)) {
      itk::ExposeMetaData<double>(dict, key, data.tmpd);
      head.label = 'F';
      head.length = 8;
    } else if(dict[key]->GetMetaDataObjectTypeInfo() == typeid(data.tmpf)) {
      itk::ExposeMetaData<float>(dict, key, data.tmpf);
      head.label = 'F';
      head.length = 4;
    } else if(dict[key]->GetMetaDataObjectTypeInfo() == typeid(data.tmpus)) {
      itk::ExposeMetaData<unsigned short>(dict, key, data.tmpus);
      head.label = 'U';
      head.length = 2;
    } else if(dict[key]->GetMetaDataObjectTypeInfo() == typeid(data.tmpsi)) {
      itk::ExposeMetaData<short>(dict, key, data.tmpsi);
      head.label = 'S';
      head.length = 2;
    } else if(dict[key]->GetMetaDataObjectTypeInfo() == typeid(data.tmpi)) {
      itk::ExposeMetaData<int>(dict, key, data.tmpi);
      head.label = 'S';
      head.length = 4;
    } else if(dict[key]->GetMetaDataObjectTypeInfo() == typeid(data.tmpu)) {
      itk::ExposeMetaData<unsigned int>(dict, key, data.tmpu);
      head.label = 'U';
      head.length = 4;
    } else if(dict[key]->GetMetaDataObjectTypeInfo() == typeid(data.tmpz)) {
      itk::ExposeMetaData<size_t>(dict, key, data.tmpz);
      head.label = 'U';
      head.length = 8;
    } else if(dict[key]->GetMetaDataObjectTypeInfo() == typeid(data.tmpll)) {
      itk::ExposeMetaData<long long int>(dict, key, data.tmpll);
      head.label = 'S';
      head.length = 8;
    } else if(dict[key]->GetMetaDataObjectTypeInfo() == typeid(data.tmpllu)) {
      itk::ExposeMetaData<long long unsigned int>(dict, key, data.tmpllu);
      head.label = 'U';
      head.length = 8;
#ifdef QUAD
    } else if(dict[key]->GetMetaDataObjectTypeInfo() == typeid(data.tmpq)) {
      itk::ExposeMetaData<quad>(dict, key, data.tmpq);
      head.label = 'F';
      head.length = 16;
#endif //QUAD
    } else {
    ...
    memcpy(buffer, &head, 2);
    memcpy(buffer+2, &data, (size_t)head.length);
  }
  return 2+head.length;
}
...
::WriteImageInformation(void)
{
  ...
  //format: <tag>\0<type>< unsigned char - num bytes><data>
  //        [len] 1[ 1  ][           1              ][0-255]
  //where type can be:
  //U - unsigned - length of 2 means short, 4 means long, 8 means long long
  //S - signed - length of 2 means short, 4 means long, 8 means long long
  //F - floating point - length of 4 means float, 8 means double, 16 means quad
  //C - char specifically an arry of length, length determines the length of the string
  MetaDataDictionary &thisDic=this->GetMetaDataDictionary();
  std::vector<std::string> keys = thisDic.GetKeys();
  for(size_t ii = 0 ; ii<keys.size() ; ii++) {
//    std::string out = printtostring(thisDic, keys[ii]);
    unsigned char buff[keys[ii].length()+3 + 255];
    memcpy(buff, keys[ii].c_str(), keys[ii].length()+1);
    size_t datalen = _writekey(thisDic, keys[ii], buff+keys[ii].length()+1, 257);
//for now use an invalid ecode until a valid one is given
//this prevents collisions
    nifti_add_extension(this->m_NiftiImage, (char*)buff, datalen+keys[ii].length()+1, 
                INVALID_ECODE);
  }
  return;
}
