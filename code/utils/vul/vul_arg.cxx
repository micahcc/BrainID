...
VDS void settype(vul_arg<vcl_vector<vcl_string> > &argmt) {argmt.type_ = "string list";}

VDS void print_value(vcl_ostream &s, vul_arg<vcl_vector<vcl_string> > const &argmt)
{
  for (unsigned int i=0; i<argmt().size(); ++i)
    s << ' ' << argmt()[i];
}

VDS int parse(vul_arg<vcl_vector<vcl_string> >* argmt, char ** argv)
{
  if ( !argv ||  !argv[0] ) {
    // no input
    vcl_cerr << "vul_arg_parse: Expected a vector of strings, none is provided.\n";
    return -1;
  }
...
template class vul_arg<vcl_vector<vcl_string> >;
